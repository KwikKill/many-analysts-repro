...
hist(data$skinrating)
 #not normally distributed, more light than dark skin players
...
#Cameron and Trivedi (2009) - robust se to control for mean not always equally variance (although pretty close here)
library(sandwich)
cov.pm <- vcovHC(poissonmodel, type = "HC0")
std.err <- sqrt(diag(cov.pm))
r.est <- cbind(Estimate = coef(poissonmodel), `Robust SE` = std.err, `Pr(>|z|)` = 2 *
    pnorm(abs(coef(poissonmodel)/std.err), lower.tail = FALSE), LL = coef(poissonmodel) - 1.96 * std.err, UL = coef(poissonmodel) + 1.96 * std.err)
...
data3all$p.darkskin = data3all$redCards.darkskin
data3all$games.darkskin
data3all$p.lightskin = data3all$redCards.lightskin
data3all$games.lightskin

#create a "relative risk" like variable (ratio of frequencies of occurances)
data3all$ratio.p.darktolight = data3all$p.darkskin/data3all$p.lightskin
 #lots of divided by 0... lose a lot of data when lightskin p = 0

hist(data3all$p.darkskin)
hist(data3all$p.lightskin)
 #one major outlier in each group...
 
data3all[which(data3all$p.lightskin > .08),]
data3all[which(data3all$p.darkskin > .08),]
 #outlier in white group is not included in analyses, because no data for dark skinned group, but other outlier has data for both, perhaps remove? 

data3all.o = subset(data3all, data3all$refCountry!=133)
hist(data3all.o$p.darkskin)

#try with relative risk statistic
...
std.err <- sqrt(diag(cov.pm))
r.est <- cbind(Estimate = coef(linearmodel.imp.o), `Robust SE` = std.err, `Pr(>|z|)` = 2 *
    pnorm(abs(coef(linearmodel.imp.o)/std.err), lower.tail = FALSE), LL = coef(linearmodel.imp.o) - 1.96 * std.err, UL = coef(linearmodel.imp.o) + 1.96 * std.err)
...
std.err <- sqrt(diag(cov.pm))
r.est <- cbind(Estimate = coef(linearmodel.exp.o), `Robust SE` = std.err, `Pr(>|z|)` = 2 *
    pnorm(abs(coef(linearmodel.exp.o)/std.err), lower.tail = FALSE), LL = coef(linearmodel.exp.o) - 1.96 * std.err, UL = coef(linearmodel.exp.o) + 1.96 * std.err)
